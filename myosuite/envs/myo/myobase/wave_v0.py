...
class WaveHandEnvV0(BaseV0):

    DEFAULT_OBS_KEYS = ['qpos', 'qvel', 'pose_err', 'phase_var',
                        'muscle_velocity', 'muscle_length', 'muscle_force']
    ...
    def _setup(self,
               ...
               # joint ranges as tuples {name:(min, max)}_nq
               target_jnt_range: dict = None,
               # desired joint vector [des_qpos]_nq
        ...
        self.wrist_period = wrist_period
        self.steps = 0
    ...
    def step(self, *args, **kwargs):
        ...
        self.steps += 1
        # self.update_target_viz(restore_sim=True)
        return obs, reward, done, info

    def get_obs_vec(self):
        self.obs_dict['time'] = np.array([self.sim.data.time])
        # Ignore the pro_sup angle
        self.obs_dict['qpos'] = self.sim.data.qpos[1:].copy()
        self.obs_dict['qvel'] = self.sim.data.qvel[:].copy()*self.dt
        if self.sim.model.na > 0:
            self.obs_dict['act'] = self.sim.data.act[:].copy()

        self.obs_dict['pose_err'] = self.target_jnt_value - \
            self.obs_dict['qpos']
    ...
    def get_obs_dict(self, sim):
        ...
        # Ignore the pro_sup angle
        obs_dict['qpos'] = sim.data.qpos[1:].copy()
        obs_dict['qvel'] = sim.data.qvel[:].copy()*self.dt
        obs_dict['act'] = sim.data.act[:].copy(
        ) if sim.model.na > 0 else np.zeros_like(obs_dict['qpos'])

        obs_dict['pose_err'] = self.target_jnt_value - obs_dict['qpos']
        obs_dict['phase_var'] = np.array(
            [(self.steps/self.wrist_period) % 1]).copy()
    ...
    def get_reward_dict(self, obs_dict):
        phase_var = (self.steps/self.wrist_period) % 1
        ...
        rwd_dict = collections.OrderedDict((
            # Optional Keys
            ('pose',    -1.*pose_dist),
            ('bonus',   1.*(pose_dist < self.pose_thd) + \
             1.*(pose_dist < 1.5*self.pose_thd)),
            ('cyclic', -1.*np.expand_dims(np.expand_dims(np.array(self._get_cyclic_wrist_rew(phase_var)), 0), 0)),
        ...
        rwd_dict['dense'] = np.sum([wt*rwd_dict[key]
                                   for key, wt in self.rwd_keys_wt.items()], axis=0)
        return rwd_dict
    ...
    def update_target_viz(self, restore_sim=False):
        if restore_sim:
        ...
        # Generate the desired pro_sup angle
        phase_var = (self.steps/self.wrist_period) % 1
        self.sim.data.qpos[0] = self._get_expected_wrist_angle(phase_var)
        self.sim.data.qpos[1:] = self.target_jnt_value.copy()
        self.sim.forward()
        ...
        if self.target_type == "generate":
            ...
            if self.target_jnt_value[0] != -0.145125:
                self.target_jnt_value = np.array(
                    [-0.145125, 0.92524251, 1.08978337, 1.39425813, -0.78286243, -0.77179383, -0.15042819, 0.64445902])
                ...
                self.target_jnt_value = np.array(
                    [-0.12756566, 0.06741454, 1.51352705, 0.91777418, -0.63884237, 0.22452487, 0.42103326, 0.4139465])
                self.sim.model.site_pos[self.target_sids[0]] = np.array(
    ...
    def _get_expected_wrist_angle(self, phase_var):
        des_angles = np.array(
            [(.77 / 2) * np.cos(phase_var * 2 * np.pi) + (.8 + (.77 / 2))], dtype=np.float32)

        return des_angles

    def _get_cyclic_wrist_rew(self, phase_var):
        """
        A rhythmic pro-supination twist is rewarded to incentivize a waving pattern.
        """
        des_angles = self._get_expected_wrist_angle(phase_var)

        angles = self._get_angle(['pro_sup'])
