...
# Create ASL envs ==============================
jnt_namesHand = ['pro_sup', 'deviation', 'flexion', 'cmc_abduction', 'cmc_flexion', 'mp_flexion', 'ip_flexion', 'mcp2_flexion', 'mcp2_abduction', 'pm2_flexion', 'md2_flexion', 'mcp3_flexion',
                 'mcp3_abduction', 'pm3_flexion', 'md3_flexion', 'mcp4_flexion', 'mcp4_abduction', 'pm4_flexion', 'md4_flexion', 'mcp5_flexion', 'mcp5_abduction', 'pm5_flexion', 'md5_flexion']
...
Rpos = {}
# Need this to avoid the pro_sup angle in the wave
Rpos_no_pro_sup = {}
for i_n, n in enumerate(jnt_namesHand):
    Rpos[n] = (np.min(m[:, i_n]), np.max(m[:, i_n]))

for i_n, n in enumerate(jnt_namesHand[1:]):
    Rpos_no_pro_sup[n] = (np.min(m[:, i_n]), np.max(m[:, i_n]))
...
# All the angles for a wave, minus the pro_sup angle, since we don't want that one to be fixed
Wave_qpos = '.03 -1 .191 -.343 -.0659 0 0 -.055 0 0 0 -.0497 0 0 0 -.0497 0 0 0 -.11 0 0'.split(
    ' ')
# Wave Env
register_env_with_variants(id='myoHandWave-v0',
                           entry_point='myosuite.envs.myo.myobase.wave_v0:WaveHandEnvV0',
                           max_episode_steps=400,
                           kwargs={
                               'model_path': curr_dir + '/../assets/hand/myohand_pose.xml',
                               'viz_site_targets': ('THtip', 'IFtip', 'MFtip', 'RFtip', 'LFtip'),
                               'target_jnt_value': np.array(Wave_qpos, 'float'),
                               # Remove the pro_sup
                               #                               'target_jnt_range': Rpos_no_pro_sup,
                               'normalize_act': True,
                               'reset_type': "random",
                               'pose_thd': .7,
                               'wrist_period': 100
...
register_env_with_variants(id='myoLegWalk-v0',
                           ...
                           kwargs={
                               ...
                               # if None then the initial root pos will be taken, otherwise provide quat
                               'target_rot': None
                           }
                           )
